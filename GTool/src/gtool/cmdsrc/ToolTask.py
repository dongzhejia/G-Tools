...
class TaskContext(object):
    ...
    def setToolConfig(self, xcfg):
        self._xcfg = xcfg

    def getToolConfig(self):
        return self._xcfg
...
def runTasks(tptask, taskItems, xcfg, execOptions):
    ...
    for task in taskItems:
        ...
        if skipTaskItems:
            ...
            ok = tptask.runTaskItem(task, task["abs_input"], task["abs_output"])
            executed = executed+1
            if ok or ok==None:
                successes = successes+1
            else:
    ...
    ok = tptask.runPostTask(taskItems)
    if ok!=None and not ok:
