...
def _switch(**args):
    ...
    oldTarget = xcfg.getValue("xtool", "target", predefs.DEFAULT_TARGET)
    newTarget = args["target"]
    if oldTarget==newTarget:
        logutil.echo("Already under the target '%s'" % (oldTarget))
    else:
        if xcfg.setValue("xtool", "target", newTarget):
...
def _list(**args):
    '''查看当前可选target'''
    logutil.echo(XConfig.getXConfig().getProjectSettings("target"))
