...
def runTasks(xtask, taskItems, xcfg, execOptions):
    ...
    def runtask(xtask, taskItem, inputs, outputs):
        if skipTaskItems:
            return True
        return xtask.runTaskItem(taskItem, inputs, outputs)
    ...
    def taskGenerator():
        for item in taskItems:
            ...
            yield {
                "name": name,
                "actions": [(runtask, (xtask, item, inputs, outputs))],
                "file_dep": xtask.dependentFiles(inputs, item),
                "targets": xtask.targetFiles(outputs, item),
