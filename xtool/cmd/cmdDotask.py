...
from . import XConfig, XTask, XTaskCollector, XDoit, logutil
...
def executeTaskItems(doargs, args, taskItems, taskInfo, taskSettings, taskContext):
    ...
    if mode=="direct":
        result = XTask.runTasks(taskObj, taskItems, xcfg, doargs)
    elif mode=="doit":
        result = XDoit.runTasks(taskObj, taskItems, xcfg, doargs)
    else:
...
def createTaskSequenceCmdEntry(seqInfo):
    ...
    def cmdEntry(ctx, **args):
        ...
        for taskName in subtasks:
            taskInfo = XTask.getTaskInfo(taskName)
            if taskInfo==None:
                continue
...
def setupTaskCommands():
    for taskName, taskInfo in XTask.registeredTasks():
    ...
    for seqName, seqInfo in XTask.taskSequences():
